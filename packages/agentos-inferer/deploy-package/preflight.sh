...
usage() {
  cat <<'EOF'
usage: packages/deploy/preflight.sh [--tag vX.Y.Z] [--repo owner/name] [--output path] [--health] [--no-github] [--allow-dirty] [--allow-git-root]
...
validator = repo_root / "packages/deploy/validate-compose.sh"
if validator.exists():
