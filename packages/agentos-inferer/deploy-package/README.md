...
- Current repo version: **`v3.0.0`** release candidate
- The four acceptance bars in `.agents/context/PRD.md` define the **v3.0** release scope.
...
- k3s GPU on NixOS: apply `packages/deploy/k8s/nvidia-device-plugin-nixos.yaml`; smoke with
  `packages/deploy/k8s/gpu-vectoradd-nixos-smoke.yaml`.
...
- `packages/deploy/k8s/qft-autoresearch-loop.yaml` defines two Jobs in the `agents` namespace:
...
`packages/deploy/docker-compose.cluster.yml` runs one NATS container per node, but those containers are not independent
coordination planes. Each node must set a unique `NODE_ID` before compose starts:

```bash
# on s001
NODE_ID=s001 AGENTS_NATS_CLUSTER_ROUTES=nats://s001:6222,nats://s002:6222 packages/deploy/deploy.sh

# on s002
NODE_ID=s002 AGENTS_NATS_CLUSTER_ROUTES=nats://s001:6222,nats://s002:6222 packages/deploy/deploy.sh
```
...
`scripts/build-release-artifacts.sh` builds the release payloads for the current
`VERSION`: daemon, manager, comms helper, harness bundle, gateway bundle,
self-improvement ratchet bundle, and `SHA256SUMS`.

The `release` GitHub workflow runs validation, uploads those artifacts, and
creates the GitHub release on `v*` tags. Its `autodeploy-agents` job requires
`AGENTS_DEPLOY_HOST`, `AGENTS_DEPLOY_USER`, and `AGENTS_DEPLOY_SSH_KEY`; when
configured, it SSHes to the deploy host, runs agents-only rollback evidence,
applies `packages/deploy/deploy-release.sh --apply`, and uploads the resulting deploy
JSON evidence. It does not reboot hosts, change GPU ownership, or touch
co-tenant services.

Run the non-invasive preflight before any manual deploy:

```bash
packages/deploy/preflight.sh --repo marius-patrik/agents --tag v3.0.0
packages/deploy/preflight.sh --repo marius-patrik/agents --tag v3.0.0 --health
```
...
- Keep `README.md` here aligned with `.agents/context/ADRs.md`, `.agents/context/PRD.md`, and `.agents/context/PLAN.md` whenever execution topology changes.
