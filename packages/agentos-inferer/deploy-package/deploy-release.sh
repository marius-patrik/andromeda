...
usage() {
  cat <<'EOF'
usage: packages/deploy/deploy-release.sh --tag vX.Y.Z [--repo owner/name] [--evidence path] [--allow-dirty] [--apply]
...
commands = {
    ...
    "build_go": ["docker", "build", "-f", "packages/deploy/Dockerfile.daemon", "-t", images["go"], *[part.replace("{image}", images["go"]) for part in build_args], "packages/engine/go"],
    "build_harness": ["docker", "build", "-f", "packages/deploy/Dockerfile.harness", "-t", images["harness"], *[part.replace("{image}", images["harness"]) for part in build_args], "."],
    "build_gateway": ["docker", "build", "-t", images["gateway"], *[part.replace("{image}", images["gateway"]) for part in build_args], "packages/gateway"],
    ...
    "compose_up": [
        ...
        *local_model_endpoint_env,
        *planned_compose_cmd, "-f", "packages/deploy/docker-compose.cluster.yml", "up", "-d", "--remove-orphans", "--wait", "--wait-timeout", "180",
    ],
    "post_health": ["packages/deploy/preflight.sh", "--repo", repository, "--tag", tag, "--health", "--no-github", "--output", str(repo / "dist" / "deploy" / tag / "post-health.json")],
    "smoke": ["packages/deploy/smoke.sh", "--output", str(repo / "dist" / "deploy" / tag / "smoke.json")],
}
report = {
    "tag": tag,
    "version": version,
