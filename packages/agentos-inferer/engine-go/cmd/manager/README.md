...
- **Go service in the engine module** (`packages/engine/go/cmd/manager`) — reuses `pkg/contracts`, `internal/ops`, and `internal/store` patterns.
...
| File | Purpose |
|------|---------|
| `packages/engine/go/cmd/manager/main.go` | Entry point |
| `packages/engine/go/internal/manager/config.go` | Config loading (env + JSON) |
| `packages/engine/go/internal/manager/github.go` | `gh` CLI wrapper for issues/branches/PRs |
| `packages/engine/go/internal/manager/gateway.go` | OpenAI-format gateway client + decomposition prompt |
| `packages/engine/go/internal/manager/daemon_client.go` | Daemon HTTP API client |
| `packages/engine/go/internal/manager/state.go` | SQLite state store (processed issues + manager runs) |
| `packages/engine/go/internal/manager/manager.go` | Core polling loop + classification + PRD/ADR/suggestion handlers |
| `packages/engine/go/internal/manager/manager_test.go` | Unit tests |
| `packages/engine/go/cmd/manager/README.md` | This document |
