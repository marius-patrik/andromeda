...
def test_ratchet_rejects_core_changes_even_when_score_improves() -> None:
    decision = decide({"score": 0.5}, {"score": 1.0}, changed_paths=["packages/agent/src/tools/registry.ts"])
...
def test_core_infra_always_blocked_pending_adr_even_when_heldout_improves() -> None:
    ...
    for core_path in (
        ...
        "packages/self_improve/ratchet.py",
    ):
...
def test_auto_merge_path_classification() -> None:
    ...
    assert not is_auto_merge_path("packages/gateway/gateway/main.py")
...
def test_changed_extension_targets_collects_all_targets_and_core_paths() -> None:
    ...
    assert ineligible == ["packages/gateway/gateway/main.py"]
...
def test_gate_rejects_mixed_core_and_extension_change(capsys) -> None:
    code = main([
        ...
        "--changed-path", "packages/gateway/gateway/main.py",
    ])
    assert code == 1
    payload = json.loads(capsys.readouterr().out)
    assert payload["decision"]["requires_human_adr"]
    assert payload["ineligible_paths"] == ["packages/gateway/gateway/main.py"]
