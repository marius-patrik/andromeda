...
async function ensureDoctorProduct(state: ReturnType<typeof tempState>): Promise<void> {
  ...
  await installCapability(state, {
    kind: "skill",
    name: "test",
    source: path.join(sourceRoot, "packages", "migrate", "agents", ".global", "skills", "test"),
  });
  ...
  for (const name of await readdir(path.join(sourceRoot, "packages", "migrate", "agents", ".global", "roles"))) {
    if (name.endsWith(".yaml")) await copyFile(path.join(sourceRoot, "packages", "migrate", "agents", ".global", "roles", name), path.join(identitySource, "roles", name));
  }
  for (const name of await readdir(path.join(sourceRoot, "packages", "migrate", "agents", ".global", "commands"))) {
    if (name.endsWith(".md")) await copyFile(path.join(sourceRoot, "packages", "migrate", "agents", ".global", "commands", name), path.join(identitySource, "prompts", name));
  }
