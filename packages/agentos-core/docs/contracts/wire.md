...
One proto module → three language stubs via **buf remote plugins** (no local
protoc needed). From the **repo root**:

```sh
buf generate proto      # uses buf.gen.yaml
# lint / breaking-change check:
buf lint proto
buf breaking proto --against '.git#branch=main,subdir=proto'
```

Outputs (all committable, all in-tree):

| Lang | Output | Plugins | Runtime dep |
|------|--------|---------|-------------|
| **Go** | `packages/agentos-core/contracts-go/gen` | `protocolbuffers/go` + `connectrpc/go` | `connectrpc.com/connect`, `google.golang.org/protobuf` (`go mod tidy`) |
| **TS** | `clients/shared-ts/src/gen` | `bufbuild/es` (protobuf-es v2) | `@bufbuild/protobuf`, `@connectrpc/connect` (`bun install`) |
| **Python** | `packages/agentos-inferer/python-packages/agentos-inferer/python-packages/agentos-inferer/python-agent/agent/gen` | `protocolbuffers/python` + `pyi` | `protobuf` (`uv sync`) |
...
- **Python:** plain protobuf `*_pb2.py` + `*_pb2.pyi`. Import as:
  ```python
...
| File | Kind | Purpose |
|------|------|---------|
| `packages/agentos-inferer/python-packages/agentos-inferer/python-packages/agentos-inferer/python-agent/agent/gen/__init__.py` | hand-authored | `sys.path` bootstrap so `from rommie.v1 import …` resolves; without it `import agent.gen` breaks |
| `packages/agentos-inferer/python-packages/agentos-inferer/python-packages/agentos-inferer/python-agent/agent/gen/rommie/__init__.py` | hand-authored | Python package marker for the `rommie` namespace |
| `packages/agentos-inferer/python-packages/agentos-inferer/python-packages/agentos-inferer/python-agent/agent/gen/rommie/v1/__init__.py` | hand-authored | Python package marker for the `rommie.v1` namespace |
| `clients/shared-ts/src/gen/index.ts` | hand-authored | Re-export barrel so consumers import from `@agentos/shared-ts/gen` |
| `packages/agentos-inferer/python-packages/agentos-inferer/python-packages/agentos-inferer/python-agent/agent/gen/rommie/v1/*_pb2.py` | **buf-generated** | Protobuf message classes — regenerate, never hand-edit |
| `packages/agentos-inferer/python-packages/agentos-inferer/python-packages/agentos-inferer/python-agent/agent/gen/rommie/v1/*_pb2.pyi` | **buf-generated** | Type stubs — regenerate, never hand-edit |
...
| `packages/agentos-core/contracts-go/gen/…` | **buf-generated** | Go message + Connect stubs — regenerate, never hand-edit |
...
buf generate proto                               # clean
(cd packages/agentos-core/contracts-go && go build ./...)     # builds
bun install                                       # green
(cd agent && uv sync && uv run python -c "import agent.gen; from rommie.v1 import session_frames_pb2")
```
