...
> **Implementation:** `agent/agent/engines/contract.py`

Every inference engine sits behind a single Python Protocol.  inferctl
(`packages/agentos-inferer/services/inferctl`, Go) is the controller; the Python contract is the seam
...
`EngineDescriptor`) live alongside the Protocol in
`agent/agent/engines/contract.py` and provide the metadata that inferctl uses
for placement decisions and gateway registry publication.  They are NOT part
of the `EngineAdapter` call surface.
...
All types live in `agent/agent/engines/contract.py` and use Python ≥ 3.12
...
1. Create `agent/agent/engines/dynamo.py`, `class DynamoAdapter`.
...
*This document tracks `agent/agent/engines/contract.py`.  It must be updated
