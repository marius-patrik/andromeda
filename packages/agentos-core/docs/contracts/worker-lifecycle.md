...
The full, typed contract is `agent/agent/conductor/lifecycle.py` —
`WorkerLifecycle` (a `typing.Protocol`).  This document describes the
*semantics*; the Python file is the authoritative *shape*.
...
(`agent/agent/conductor/`) to a standalone Go service (`services/conductor`),
...
| Path | Content |
|---|---|
| `agent/agent/conductor/__init__.py` | Package init; re-exports all public names. |
| `agent/agent/conductor/lifecycle.py` | Typed contract: all types + `WorkerLifecycle` Protocol. |
| `docs/contracts/worker-lifecycle.md` | This document. |

Implementation files (VS0 stub, later Go adapter) live under
`agent/agent/conductor/` and are **not** part of this contract.
