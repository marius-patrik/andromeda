...
test("monorepo validation uses the uv CLI without a cross-repository go.work", async () => {
  const commands = await readFile("packages/migrate/agents/.project/COMMANDS.md", "utf8");
...
test("legacy provider-specific review assets are absent and no longer required", async () => {
  const legacyPaths = [
    ...
    "packages/scripts/run-codex-review.sh",
    "packages/scripts/run-kimi-review.mjs",
