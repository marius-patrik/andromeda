...
test("denied failure: conflicted managed package index entries cannot pass", () => {
  const target = fixture();
  try {
    git(target, "update-index", "--force-remove", "packages/migrate/agents/darkfactory");
    addIndexEntries(
      target,
      `160000 ${fixtureGitlinkOid} 1\tpackages/migrate/agents/darkfactory\n160000 ${fixtureGitlinkOid} 2\tpackages/migrate/agents/darkfactory\n`,
    );
    const issues = inventoryIssues(target).join("\n");
    assert.match(issues, /managed package has multiple index entries: packages\/migrate\/agents\/darkfactory/);
    assert.match(issues, /managed package has unmerged index entries: packages\/migrate\/agents\/darkfactory/);
  } finally {
    rmSync(target, { recursive: true, force: true });
  }
});
