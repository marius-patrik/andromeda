...
const suites = {
  ...
  core() {
    ...
    run("generated contract freshness", "bun", ["packages/scripts/verify-codegen.ts"]);
  ...
  release() {
    run("installer and release smoke", "bun", ["packages/scripts/run-release-smoke.mjs"]);
  },
  sync() {
    run("encrypted event-exchange smoke", "bun", ["packages/scripts/run-sync-smoke.mjs"]);
  },
  review() {
