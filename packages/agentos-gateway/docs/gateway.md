# Gateway (packages/agentos-gateway) — VS1-minimal salvage + VS2 backlog
...
is **copied + adapted** into `packages/agentos-gateway` as `agentos_gateway`. The v3
...
6. **`pyproject.toml`** (uv, py>=3.12) so
   `cd packages/agentos-gateway && uv sync && uv run pytest` is green.
...
- **The `inferctl` seam** (§13 #4): inferctl (Go, in `packages/agentos-inferer/services/inferctl`) brings
