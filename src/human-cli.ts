...
export const HUMAN_COMMANDS: readonly CommandSpec[] = Object.freeze([
  ...
  command({
    id: "issue-draft", path: ["issue", "draft"], usage: "df issue draft [owner/repo] [--effort low|medium|high] [--input FILE] [--draft FILE] [--resume] [--approve SHA256] [--json]", purpose: "Gather intent interactively, compose a high-tier local issue draft, run issue Autoreview/autofix, show the final diff, and publish only after exact human approval.",
    defaults: "Control repository; high model tier; policy effort; local reviewed draft only until the exact reviewed digest is approved. Reviewed drafts expire under the versioned local hygiene policy.", model: "Tier high is fixed by policy; --effort independently selects the supported effort setting. Expired review evidence is discarded only by explicit owner resume, then rebuilt through a fresh high confirmation.", permissions: "Repository contents read; issue write only after reviewed-digest approval; darkfactory-data ledger write.",
    mutations: "Before approval only the local draft and durable model/lifecycle receipts change. Approval may create exactly one issue bound to the fresh reviewed draft digest; hygiene never drafts, publishes, or deletes content.", trust: "Canonical Agent OS launcher, exact control revision, live repository inventory, versioned prompt and draft-hygiene policies, and durable receipts are required. Untrusted intent never grants authority.",
    examples: ["df issue draft marius-patrik/DarkFactory", "df issue draft marius-patrik/DarkFactory --input intent.json --draft draft.json --json", "df issue draft --draft draft.json --resume", "df issue draft --draft draft.json --approve 8f1c..."],
    failures: "Unavailable Agent OS, malformed output, unresolved owner decisions, expired or stale review evidence, failed review, receipt failure, or approval mismatch leaves the issue unpublished. Use --resume only for an expired reviewed draft.",
    options: [JSON_OPTION, { name: "--effort", value: "LEVEL", description: "Select reasoning effort independently of the fixed high model tier." }, { name: "--input", value: "FILE", description: "Read structured owner answers instead of prompting." }, { name: "--draft", value: "FILE", description: "Create or resume one exact local draft-state file." }, { name: "--resume", description: "Explicitly resume an expired reviewed draft and require a fresh high confirmation." }, { name: "--approve", value: "SHA256", description: "Approve publication of exactly this fresh reviewed draft digest." }],
