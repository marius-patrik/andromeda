...
This document specifies the execution-lane interface contract for Andromeda. This seam allows the Andromeda 4.0 runtime to decouple capability execution from the underlying infrastructure, facilitating detached execution on k3s Jobs + NATS + Daemon today, and enabling Knative (or other FaaS engines) to drop in post-4.0 with zero runtime code changes.

The contract is implemented entirely within python package [agent.exec_lane](../../agent/agent/exec_lane/__init__.py) (specifically [contract.py](../../agent/agent/exec_lane/contract.py)) using typed Python stdlib only.
