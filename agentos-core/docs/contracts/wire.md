...
|------|--------|---------|-------------|
| **Go** | `agentos-core/contracts-go/gen` | `protocolbuffers/go` + `connectrpc/go` | `connectrpc.com/connect`, `google.golang.org/protobuf` (`go mod tidy`) |
| **TS** | `clients/shared-ts/src/gen` | `bufbuild/es` (protobuf-es v2) | `@bufbuild/protobuf`, `@connectrpc/connect` (`bun install`) |
| **Python** | `inference-engine/python-inference-engine/python-inference-engine/python-agent/agent/gen` | `protocolbuffers/python` + `pyi` | `protobuf` (`uv sync`) |
...
| File | Kind | Purpose |
|------|------|---------|
| `inference-engine/python-inference-engine/python-inference-engine/python-agent/agent/gen/__init__.py` | hand-authored | `sys.path` bootstrap so `from rommie.v1 import …` resolves; without it `import agent.gen` breaks |
| `inference-engine/python-inference-engine/python-inference-engine/python-agent/agent/gen/rommie/__init__.py` | hand-authored | Python package marker for the `rommie` namespace |
| `inference-engine/python-inference-engine/python-inference-engine/python-agent/agent/gen/rommie/v1/__init__.py` | hand-authored | Python package marker for the `rommie.v1` namespace |
| `clients/shared-ts/src/gen/index.ts` | hand-authored | Re-export barrel so consumers import from `@agentos/shared-ts/gen` |
| `inference-engine/python-inference-engine/python-inference-engine/python-agent/agent/gen/rommie/v1/*_pb2.py` | **buf-generated** | Protobuf message classes — regenerate, never hand-edit |
| `inference-engine/python-inference-engine/python-inference-engine/python-agent/agent/gen/rommie/v1/*_pb2.pyi` | **buf-generated** | Type stubs — regenerate, never hand-edit |
...
| `agentos-core/contracts-go/gen/…` | **buf-generated** | Go message + Connect stubs — regenerate, never hand-edit |
...
(cd agentos-core/contracts-go && go build ./...)     # builds
bun install                                       # green
